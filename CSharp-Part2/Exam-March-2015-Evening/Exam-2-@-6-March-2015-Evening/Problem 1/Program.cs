...
namespace Problem_1
{
    class Program
    {
        ...
        public static char[] ToBaseD(char[] toDecimal, int d)
        {
            ...
            power--;
            ulong hexDigit = tempDecimal / (ulong)MathPow(d, power);
            tempDecimal -= hexDigit * (ulong)MathPow(d, power);
            ...
            if (tempDecimal != 0)
            {
                for (; power > -1; power--)
                {
                    if (tempDecimal < (ulong)MathPow(d, power))
                    {
                        tempResult.Add(0);
                        continue;
                    }
                    hexDigit = tempDecimal / (ulong)MathPow(d, power);
                    tempDecimal -= hexDigit * (ulong)MathPow(d, power);
                    tempResult.Add(hexDigit);
                }
            }
        ...
        public static char[] ToDecimal(char[] number, int s)
        {
            ...
            for (int i = 0; i < number.Length; i++)
            {
                ...
                tempResult += (ulong)value * (ulong)MathPow(s, i);
            }
